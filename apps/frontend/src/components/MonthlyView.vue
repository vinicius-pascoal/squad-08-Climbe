<template>
  <div class="monthly-view">
    <div class="grid grid-cols-7 gap-1 text-xs text-slate-600 mb-2">
      <div v-for="d in ['Dom','Seg','Ter','Qua','Qui','Sex','Sab']" :key="d" class="text-center font-semibold">{{ d }}</div>
    </div>

    <div class="grid grid-cols-7 gap-2">
      <div v-for="cell in cells" :key="cell.key" class="min-h-24 p-2 border rounded bg-white">
        <div class="text-sm font-semibold mb-1">{{ cell.label }}</div>
        <ul class="space-y-1">
          <li v-for="ev in cell.events" :key="ev.id" class="text-xs p-1 rounded hover:bg-slate-100 cursor-pointer" @click="$emit('event-click', ev)">
            <div class="truncate font-medium">{{ ev.summary || ev.title }}</div>
            <div class="text-[11px] text-slate-500">{{ ev.timeLabel }}</div>
          </li>
        </ul>
      </div>
    </div>
